...
def test_read_all_has_jack(logger):
    """
    Test on hitting People Get API, we get user named Jack in the list of people
    :param logger:
    :return:
    """
    ...
    assert_people_have_person_with_first_name(response, first_name='Jack')


def test_read_all_has_santa():
    response = client.read_all_persons()

    assert_that(response.status_code).is_equal_to(requests.codes.ok)
    assert_people_have_person_with_first_name(response, first_name='Santa')
